...
const NEW_EMBEDDING_FIELD = "title_embedding_qwen3";
const BATCH_SIZE = 50;
...
async function main() {
  try {
    ...
    for await (const doc of collection.find(query)) {
      ...
      const vector = await embeddings.embedQuery(doc["Job Title"]); // use title
      console.log(doc["Job Title"].length);
      console.log("here");
